import{r as l,b,j as d,L as f,i as r,c as h}from"./index-acTPG1pN.js";import{b as m}from"./inventoryPricing-ScrDhAis.js";const A=`
...
`,_=()=>{const[i,g]=l.useState(!0);if(l.useEffect(()=>{b({force:!0}).catch(()=>{})},[]),l.useEffect(()=>{(async()=>{window.__CONFIG_DATA__={i18n:r,baseControllerPrice:0,baseControllerQty:null,baseControllerLowStockThreshold:5},window.__CONFIG_FIREBASE_DATA__=window.__CONFIG_FIREBASE_DATA__||[];try{const t=await h(),u=(t.parts||[]).map(p=>({...p,options:(p.options||[]).map(c=>{const a=c;return{id:a.id,...a,...m(a.inventoryDetails,{purchasePrice:a.purchasePrice??0,sellPrice:a.sellPrice??a.price??0},{quantity:a.quantity??0})}}).filter(c=>c.active!==!1)})),o=Number(t.basePrice)||0,n=t.baseQuantity!=null?Number(t.baseQuantity):null,s=Number(t.baseControllerLowStockThreshold),v=Number.isFinite(s)&&s>=0?s:5;window.__CONFIG_FIREBASE_DATA__=u,window.__CONFIG_DATA__={i18n:r,baseControllerPrice:o,baseControllerQty:n,baseControllerLowStockThreshold:v}}catch(t){console.error("Configurator catalog fetch error:",t),window.__CONFIG_FIREBASE_DATA__=[]}finally{g(!1)}})()},[]),l.useEffect(()=>{if(i)return;document.body.classList.add("configurator-page-active");const e=window.requestAnimationFrame(()=>{document.body.classList.add("configurator-intro-active")}),t=window.setTimeout(()=>{document.body.classList.remove("configurator-intro-active")},1350);if(!document.getElementById("ez-configurator-logic-script")){const o=document.createElement("script");o.id="ez-configurator-logic-script",o.src="/configurator-logic.js?v="+Date.now(),o.async=!0,document.body.appendChild(o)}return()=>{if(typeof window.__EZ_CONFIGURATOR_CLEANUP__=="function")try{window.__EZ_CONFIGURATOR_CLEANUP__()}catch(n){console.warn("Configurator cleanup failed",n)}document.body.classList.remove("configurator-page-active"),document.body.classList.remove("configurator-intro-active"),window.cancelAnimationFrame(e),window.clearTimeout(t);const o=document.getElementById("ez-configurator-logic-script");o&&document.body.contains(o)&&document.body.removeChild(o)}},[i]),i){const e=localStorage.getItem("ez_lang")||"ar";return d.jsx(f,{message:r[e]&&r[e].loadingConfigurator||"Loading configurator...",fullScreen:!0})}return d.jsx("div",{className:"configurator-page",children:d.jsx("div",{dangerouslySetInnerHTML:{__html:A}})})};export{_ as default};
